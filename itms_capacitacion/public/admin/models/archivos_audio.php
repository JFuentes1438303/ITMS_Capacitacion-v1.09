...
<?php  
	...
	$sql2 = "SELECT * FROM audio LIMIT $inicio_registro, $por_pagina";

	$resultado2 = $conexion -> prepare($sql2);

	$resultado2 -> execute(array());
	while ($registro2 = $resultado2 -> fetch(PDO::FETCH_ASSOC)) {
		$nnombre = stripcslashes($registro2["nombre_audio"]);
		$rruta = stripcslashes($registro2["ruta"]);
		$iid = stripcslashes($registro2["id_audio"]);
	...
	echo "<div class='col-sm-4 t_centro'>";
		echo "<a href='../../temporal/audios/$rruta' class='btn btn-sm b_descargar' onclick='return descargar()' title='Descargar Archivo' download='$nnombre'><i class='fas fa-download'></i></a>";
	echo "</div>";
	echo "</div>";
	echo "</form>";
	}	
...
<div class="row centro">
	...
	<?php  
		...
		echo "<div class='row centro'>";
			...
			if ($pagina_actual != 1) {
				...
				echo "<li><a class='btn btn-sm page-link flecha' href='archivos_audio?pagina=".($pagina_actual-1)."'>";
			...
			for ($i=1; $i <= $total_paginas; $i++) { 
				if($pagina_actual == $i){
					echo "<li><a class='btn btn-sm page-link pagina active' href='archivos_audio?pagina=".$i."'>".$i."</a>";
				}else{
					echo "<li><a class='btn btn-sm page-link pagina' href='archivos_audio?pagina=".$i."'>".$i."</a>";
				}
			}
	...
	<div class="col-sm-4 t_centro links2">
		<a href="../../admin/views/home.php" class="links">Volver</a>
	</div>
</div>
</div>
<br>
